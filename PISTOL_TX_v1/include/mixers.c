...
void AETR_MID_THR() { //default mixer 
 
  throttle = map(analogRead(joystick_T), 2050, 4095, RC_CHANNEL_MIN, RC_CHANNEL_MAX);
  throttle = constrain(throttle, RC_CHANNEL_MIN, RC_CHANNEL_MAX);
  yaw = map(analogRead(joystick_Y), 4090, 5, RC_CHANNEL_MIN, RC_CHANNEL_MAX);
...
*/
  throttle = map(analogRead(joystick_T), 5, 4095, RC_CHANNEL_MIN, RC_CHANNEL_MAX);
  yaw = map(analogRead(joystick_Y), 4090, 5, RC_CHANNEL_MIN, RC_CHANNEL_MAX);
...
#ifdef SCHOOL
  const char* mixer_labels[number_of_mixers] = {"AETR MID THR", "AETR1234", "servo skid","drift car" ,"CALIB"};
#endif
